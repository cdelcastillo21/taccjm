...
def _check_init(jm_id):
    ...
    if jm_id not in JM.keys():
        raise falcon.HTTPError(
            falcon.HTTP_404, "jm_error", f"TACCJM {jm_id} does not exist."
        )


@hug.post("/init")
def init_jm(
    jm_id: str, system: str, user: str, psw: str, mfa: str, restart: bool = False
):
    ...
    if jm_id not in JM.keys() or restart:
        try:
            ...
            JM[jm_id] = TACCJobManager(
                system, user=user, psw=psw, mfa=mfa, working_dir=jm_id
            )
        ...
        raise falcon.HTTPError(
            falcon.HTTP_409, "jm_error", f"TACCJM {jm_id} already exists."
        )
        ...
        out.append(
            {
                ...
                "sys": JM[jm].system,
                "user": JM[jm].user,
                "apps_dir": JM[jm].apps_dir,
                "jobs_dir": JM[jm].jobs_dir,
            }
        )
    return out
...
def deploy_job(
    ...
    **kwargs,
    ...
    return JM[jm_id].deploy_job(
        job_config=None if job_config is None else json.loads(job_config),
...
def upload_job_file(
    ...
    JM[jm_id].upload_job_file(job_id, path, dest_dir=dest_dir, file_filter=file_filter)
...
def run_script(
    jm_id: str, script_name: str, job_id: str = None, args: hug.types.multiple = []
):
    """Run Script"""
    _check_init(jm_id)

    logger.info(f"Executing script {script_name} in background.")
    script_config = JM[jm_id].run_script(
        script_name, job_id=job_id, args=args, wait=False
    )
    res = {
        f"{i}": script_config[i]
        for i in script_config
        if i not in ["channel", "history", "cmd"]
    }
