...
class EnsembleTACCSimulation(TACCSimulation):
    ...
    def __init__(
        ...
        super().__init__(
            name, system, log_config, script_file=__file__, class_name=__name__
        )
    ...
    def run_job(self):
        ...
        if "task_file" in self.job_config["args"]:
            tasks.append(self.client.read(self.job_config["args"]["task_file"]))
        ...
        tq = SLURMTaskQueue(
            workdir=self.client.job_path(self.job_config["job_id"], "task-queue"),
        ...
        while len(tasks) > 0:
            self.log.info(
                f"Putting {len(tasks)} into queue",
